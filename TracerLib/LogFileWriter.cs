...
namespace TracerLib {
  ...
  /// <summary>
  /// LogFileWriter writes strings into a file. The file can grow only to a certain limit, which is set with the fileParameter in the constructor. 
  /// When this limit is reached, a new file gets created. If there are too many files, the oldest get deleted.
  /// 
  /// The messages get first stored in a queue, which makes writing to LogFileWriter very fast (basically none blocking). The messages get collected 
  /// for few seconds, then the file opened, the messages written and the file closed again. This allows others (Notepad) to
  /// access the log file too. If after few seconds LogFileWriter tries to write again, but the log file is blocked by another application (Notepad),
  /// then LogFileWriter will continue to log in a new file.
  /// 
  ...
  public class LogFileWriter: IDisposable {
    ...
    /// <summary>
    /// Write Message to the file with the date stamp base on system time, followed
    /// by new line
    /// </summary>
